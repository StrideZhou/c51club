...
LUA_API void lua_replace (lua_State *L, int idx) {
  StkId o;
  lua_lock(L);
  /* explicit test for incompatible code_ */
...
/*
** `load' and `call' functions (run Lua code_)
*/
...
*/
struct CCallS {  /* data_ to `f_Ccall' */
  lua_CFunction func;
  void *ud;
};
...
LUA_API int lua_load (lua_State *L, lua_Reader reader, void *data_,
                      const char *chunkname) {
  ZIO z;
  int status;
  lua_lock(L);
  if (!chunkname) chunkname = "?";
  luaZ_init(L, &z, reader, data_);
...
LUA_API int lua_dump (lua_State *L, lua_Writer writer, void *data_) {
  int status;
  TValue *o;
  lua_lock(L);
  api_checknelems(L, 1);
  o = L->top - 1;
  if (isLfunction(o))
    status = luaU_dump(L, clvalue(o)->l.p, writer, data_, 0);
  else
    status = 1;
  lua_unlock(L);
  return status;
...
LUA_API int lua_gc (lua_State *L, int what, int data_) {
  int res = 0;
  global_State *g;
  lua_lock(L);
  g = G(L);
  switch (what) {
    ...
    case LUA_GCSETMEMLIMIT: {
      /* GC values are expressed in Kbytes: #bytes/2^10 */
      lu_mem new_memlimit = (cast(lu_mem, data_) << 10);
